...
DOCKER_METRICS_URL="https://raw.githubusercontent.com/docker/engine/master/contrib/prometheus-docker-collector-scripts/docker-container-metrics"
USERNAME="yalefox"
...
install_prerequisites() {
    ...
    apt -y install net-tools zsh curl git fonts-powerline
...
install_docker_metrics() {
    print_info "Installing Docker metrics script for Prometheus..."
    if [ ! -f /usr/local/bin/docker-container-metrics ]; then
        curl -fsSL "$DOCKER_METRICS_URL" -o /usr/local/bin/docker-container-metrics
        chmod +x /usr/local/bin/docker-container-metrics
        print_status "Docker metrics script installed"
    else
        print_status "Docker metrics script is already installed"
    fi

    print_info "Setting up systemd timer for Docker metrics..."
    cat <<EOF >/etc/systemd/system/docker-container-metrics.service
[Unit]
Description=Docker Container Metrics for Prometheus
Wants=docker-container-metrics.timer

[Service]
ExecStart=/usr/local/bin/docker-container-metrics

[Install]
WantedBy=multi-user.target
EOF

    cat <<EOF >/etc/systemd/system/docker-container-metrics.timer
[Unit]
Description=Run Docker Container Metrics every minute

[Timer]
OnBootSec=2min
OnUnitActiveSec=1min
Unit=docker-container-metrics.service
...
EOF
    ...
    systemctl enable docker-container-metrics.timer
    systemctl start docker-container-metrics.timer
    print_status "Docker metrics systemd timer setup completed"
}
